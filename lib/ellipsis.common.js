...
// CONCATENATED MODULE: ./src/components/ellipsis.js
...
// The global cache.
var cache = new Map();
var proxy = document.querySelector('#vue-ellipsis-proxy');
...
var ellipsis = {
   ...
   * @return {boolean}
   */
  validate: function validate(rows, position, scale) {
    if (['start', 'middle', 'end'].indexOf(position) === -1) {
      raiseError("Invalid ellipse position value \"".concat(position, "\", available: start, middle, end"));
      return false;
    } // Position with value "middle" makes no sense if there are more than one lines


    if (rows > 1 && position === 'middle') {
      raiseError("Accept single row while position is \"middle\", got value \"".concat(rows, "\""));
      return false;
    } // Scaling makes no sense if there are more than one lines


    if (rows > 1 && scale) {
      raiseError("Accept single row while \"scale\" enabled, got value \"".concat(rows, "\""));
      return false;
    }
  ...
  destroy: function destroy(id) {
    ...
    var _cache$get = cache.get(id),
        contentProxy = _cache$get.contentProxy,
        fillProxy = _cache$get.fillProxy,
        wordProxy = _cache$get.wordProxy;
   ...
   */
  createProxy: function createProxy(parent, id, type) {
   ...
   * @return {any}
   */
  getProxy: function getProxy(el, id) {
    if (cache.has(id)) {
      return cache.get(id);
    }
   ...
   * @param style
   */
  setProxyStyle: function setProxyStyle(el, style) {
  ...
  isAlphabet: function isAlphabet(ch) {
   ...
   * @param index
   */
  getPrevWord: function getPrevWord(content, index) {
   ...
   * @param index
   */
  getNextWord: function getNextWord(content, index) {
    if (this.isSeparator(content[index])) {
      return content[index];
    }
   ...
   * @return {number}
   */
  getWordWidth: function getWordWidth(wordProxy, word) {
    // 使用 &nbsp; 作为空格来计算字符长度
   ...
   * @return {number}
   */
  getContainerWidth: function getContainerWidth(el, style) {
    if (!el) {
      return 0;
    }
   ...
   */
  getMeta: function getMeta(el, id, _ref) {
    ...
    var _this$getProxy = this.getProxy(el, id),
        wordProxy = _this$getProxy.wordProxy,
        contentProxy = _this$getProxy.contentProxy,
        fillProxy = _this$getProxy.fillProxy;

    contentProxy.innerHTML = content;
    var containerStyle = this.getStyle(el);
    ...
    var contentWidth = this.getRect(contentProxy).width;
    var containerWidth = this.getContainerWidth(el, containerStyle);
    var containerMaxWidth = parseFloat(containerStyle.maxWidth);
    var containerWordBreak = containerStyle.wordBreak === 'break-all'; // If container width not available, ellipsis won't work
    ...
    return {
      ...
      containerWordBreak: containerWordBreak,
   ...
   * @return {string}
   */
  clearContent: function clearContent(content) {
  ...
  getScaleInfo: function getScaleInfo(_ref2) {
    var containerWidth = _ref2.containerWidth,
        contentProxy = _ref2.contentProxy,
        contentWidth = _ref2.contentWidth,
        containerStyle = _ref2.containerStyle;
    // 原始高度
    var height = this.getRect(contentProxy).height;
    var fontsize = parseFloat(containerStyle.fontSize);
    var color = containerStyle.color; // 缩放比例

    var ratio = containerWidth / contentWidth;
    var scaledFontsize = fontsize * ratio;
    return {
      // 基准线，用于使文字垂直居中 (被 dominant-baseline: central 使用)
      baseline: height / 2,
      // 文字样式
      style: "fill: ".concat(color, ";font-size: ").concat(scaledFontsize, "px;dominant-baseline: central;"),
      viewBox: "0 0 ".concat(containerWidth, " ").concat(height),
   ...
   * @return {string}
   */
  makeLeft: function makeLeft(_ref3, _ref4) {
    var containerWidth = _ref3.containerWidth,
        fillWidth = _ref3.fillWidth,
        wordProxy = _ref3.wordProxy,
        containerWordBreak = _ref3.containerWordBreak;
    ...
    for (var row = rows; row >= 1; row--) {
      ...
      var size = row === 1 ? containerWidth - fillWidth : containerWidth;

      for (; i >= 0;) {
        var word = containerWordBreak ? content[i] : this.getPrevWord(content, i);
        i -= word.length;
    ...
    return "".concat(fill).concat(suffix);
  },
   ...
   * @return {string}
   */
  makeCenter: function makeCenter(_ref5, _ref6) {
    var containerWidth = _ref5.containerWidth,
        fillWidth = _ref5.fillWidth,
        wordProxy = _ref5.wordProxy;
    var content = _ref6.content,
        fill = _ref6.fill;
    var size = containerWidth - fillWidth;
    ...
    return "".concat(prefix).concat(fill).concat(suffix);
  },
   ...
   * @return {string}
   */
  makeRight: function makeRight(_ref7, _ref8) {
    var containerWidth = _ref7.containerWidth,
        fillWidth = _ref7.fillWidth,
        wordProxy = _ref7.wordProxy,
        containerWordBreak = _ref7.containerWordBreak;
    ...
    for (var row = 0; row < rows; row++) {
      ...
      var size = row === rows - 1 ? containerWidth - fillWidth : containerWidth;

      for (; i < contentLength;) {
        var word = containerWordBreak ? content[i] : this.getNextWord(content, i);
        i += word.length;
        size -= this.getWordWidth(wordProxy, word);
    ...
    return "".concat(prefix).concat(fill);
  },
   ...
   */
  make: function make(meta, option) {
    if (meta.contentWidth <= meta.containerWidth || meta.contentWidth + meta.fillWidth <= meta.containerWidth) {
  ...
  newId: function newId() {
    return "".concat(new Date().getTime()).concat(Math.round(Math.random() * 1000));
  }
};
ellipsis.init();
/* harmony default export */ var components_ellipsis = (ellipsis);
// CONCATENATED MODULE: ./src/components/directive.js
...
function makeEllipsis(el, text, position, rows, modifiers) {
  var fill = el.dataset.ellipsis;
  fill = fill ? fill : '...';
  var id = components_ellipsis.newId();
  ...
  var meta = components_ellipsis.getMeta(el, id, option);

  if (rows === 0 || !Object.keys(meta).length) {
    directive_doRender(el, false, text, text, modifiers, id);
    return;
  }
  ...
  if (modifiers.scale) {
    var _makeSvg = directive_makeSvg(meta, option);

    var _makeSvg2 = _slicedToArray(_makeSvg, 2);

    hasEllipsis = _makeSvg2[0];
    ellipsisContent = _makeSvg2[1];
  } else {
    var _ellipsis$make = components_ellipsis.make(meta, option);

    var _ellipsis$make2 = _slicedToArray(_ellipsis$make, 2);

    hasEllipsis = _ellipsis$make2[0];
    ellipsisContent = _ellipsis$make2[1];
  }

  directive_doRender(el, hasEllipsis, text, ellipsisContent, modifiers, id);
}

function directive_doRender(el, hasEllipsis, rawText, ellipsisContent, modifiers, id) {
  components_ellipsis.destroy(id);
...
function directive_makeSvg(meta, _ref) {
  var content = _ref.content;

  var _ellipsis$getScaleInf = components_ellipsis.getScaleInfo(meta),
      baseline = _ellipsis$getScaleInf.baseline,
      viewBox = _ellipsis$getScaleInf.viewBox,
      style = _ellipsis$getScaleInf.style,
      scaled = _ellipsis$getScaleInf.scaled;

  return [scaled, "<svg viewBox=\"".concat(viewBox, "\"><text x=\"0\" y=\"").concat(baseline, "\" style=\"").concat(style, "\">").concat(content, "</text></svg>")];
}

function getText(vnode) {
  var temp = [];
  vnode.children.forEach(function (i) {
    temp.push(i.text);
...
function directive_render(el, _ref2, vnode) {
  ...
  if (!components_ellipsis.validate(rows, position, modifiers.scale)) {
    return;
  }

  var text = getText(vnode);
  ...
  if (value === 0) {
    el.innerHTML = text; // reset title: Show title when modifiers.always is true
  ...
  var rows = value || 1;
  text = components_ellipsis.clearContent(text);
...
function destroy(el) {
  components_ellipsis.destroy(el);
}

/* harmony default export */ var directive = ({
  inserted: directive_render,
  update: directive_render,
  unbind: destroy,
  name: 'ellipsis'
});
...
/* harmony default export */ var component = ({
  ...
  data: function data() {
    return {
      id: components_ellipsis.newId()
    };
  },
  mounted: function mounted() {
    components_ellipsis.validate(this.rows, this.position, this.scale);
  ...
  render: function render(createElement) {
    var text = components_ellipsis.clearContent(this.content || this.getText());
    var option = {
      content: text,
      fill: this.fill,
      rows: this.rows,
    ...
    var meta = components_ellipsis.getMeta(this.$el, this.id, option);
    ...
    if (this.scale) {
      var _this$makeSvg = this.makeSvg(createElement, meta, option);

      var _this$makeSvg2 = _slicedToArray(_this$makeSvg, 2);

      hasEllipsis = _this$makeSvg2[0];
      ellipsisContent = _this$makeSvg2[1];
    } else {
      var _ellipsis$make = components_ellipsis.make(meta, option);
  ...
  methods: {
    doRender: function doRender(createElement, hasEllipsis, rawText, ellipsisContent) {
      components_ellipsis.destroy(this.id);
      var title = undefined;
    ...
    makeSvg: function makeSvg(h, meta, _ref) {
      var content = _ref.content;
...
/* concated harmony reexport ellipsisDirective */__webpack_require__.d(__webpack_exports__, "ellipsisDirective", function() { return directive; });
/* concated harmony reexport ellipsisComponent */__webpack_require__.d(__webpack_exports__, "ellipsisComponent", function() { return component; });
