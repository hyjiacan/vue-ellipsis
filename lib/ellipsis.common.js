...
function autoSize(content, contentProxy, contentWidth, containerWidth, containerStyle) {
  // 原始的高度
  var height = parseFloat(getStyle(contentProxy).height);
  var rate = containerWidth / contentWidth;
  var fontsize = parseFloat(containerStyle.fontSize);
  var scale = fontsize * rate; // // 使用二分法
  // let offset = 0
  ...
  return [Math.round(fontsize) > Math.round(scale), "<svg viewBox=\"0 -".concat(height / 2, "0 ").concat(containerWidth, " ").concat(height, "\" height=\"").concat(height, "px\"><text x=\"0\" y=\"0\" style=\"font-size: ").concat(scale, "px;\">").concat(content, "</text></svg>")];
}
/**
 *
 * @param el
...
function makeEllipsis(el, content, position, fill, rows, scale) {
  // 设置样式
  var _getProxy = getProxy(el),
      wordProxy = _getProxy.wordProxy,
      contentProxy = _getProxy.contentProxy,
      fillProxy = _getProxy.fillProxy;

  contentProxy.innerHTML = content;
  var containerStyle = getStyle(el);
  ...
  var contentWidth = parseFloat(getStyle(contentProxy).width);
  ...
  switch (position) {
    ...
    default:
      throw new Error("Invalid ellipse position value \"".concat(position, "\", available: start, middle, end"));
  }
...
function makeLeftEllipsis(containerWidth, content, fill, fillWidth, wordProxy, rows, wordBreak) {
  ...
  for (var row = rows; row >= 1; row--) {
    ...
    for (; i >= 0;) {
      var word = wordBreak ? content[i] : getPrevWord(content, i);
      i -= word.length;
...
function makeRightEllipsis(containerWidth, content, fill, fillWidth, wordProxy, rows, wordBreak) {
  ...
  for (var row = 0; row < rows; row++) {
    ...
    for (; i < contentLength;) {
      var word = wordBreak ? content[i] : getNextWord(content, i);
      i += word.length;
      size -= getWordWidth(wordProxy, word);
...
function directive_makeEllipsis(el, text, position, rows, modifiers) {
  var fill = el.dataset.ellipsis;
  fill = fill ? fill : '...';

  var _ellipsis$make = ellipsis.make(el, text, position, fill, rows, modifiers.scale),
      _ellipsis$make2 = _slicedToArray(_ellipsis$make, 2),
      hasEllipsis = _ellipsis$make2[0],
      content = _ellipsis$make2[1];
...
function render(el, _ref, vnode) {
  ...
  if (vnode.children.length !== 1) {
    console.warn("Ellipsis accept one text node only, got \"".concat(vnode.children.length, "\" children"));
    return;
  }

  if (vnode.children[0].elm) {
    if (vnode.children[0].elm.nodeType !== 3) {
      console.warn("Ellipsis accept text node only, got \"".concat(vnode.children[0].elm.nodeName, "\""));
      return;
    }
  }

  var text = vnode.children[0].text;
  ...
  if (!modifiers.delay) {
    directive_makeEllipsis(el, text, position, rows, modifiers);
    return;
  }
...
/******/ });
//# sourceMappingURL=ellipsis.common.js.map