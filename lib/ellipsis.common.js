...
function render(el, _ref, vnode) {
  ...
  if (vnode.children.length !== 1) {
    throw new Error("Ellipsis accept one text node only, got \"".concat(vnode.children.length, "\" children"));
  }

  if (vnode.children[0].elm) {
    if (vnode.children[0].elm.nodeType !== 3) {
      throw new Error("Ellipsis accept text node only, got \"".concat(vnode.children[0].elm.nodeName, "\""));
    }
  }
  ...
  if (rows > 1 && position === 'middle') {
    throw new Error("Ellipsis accept single row while position is \"middle\", got value \"".concat(rows, "\""));
  }

  var fill = el.dataset.ellipsis;
  fill = fill ? fill : '...';

  var _ellipsis$make = ellipsis.make(el, text, position, fill, rows),
